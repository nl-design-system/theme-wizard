...
describe('/get-css', () => {
  describe('query param validation', () => {
    test('missing `url` query param', async () => {
      const response = await app.request('/v1/api/get-css');
    ...
    test('empty `url` query param', async () => {
      const response = await app.request('/v1/api/get-css?url=');
      expect.soft(response.status).toBe(400);
      expect.soft(await response.text()).toBe('missing `url` parameter: specify a url like ?url=example.com');
    });
  });
  ...
  describe('fetching css', () => {
    describe('happy path', () => {
      ...
      test('returns a string of css', async () => {
        ...
        const response = await app.request('/v1/api/get-css?url=example.com');
    ...
    describe('error handling', () => {
      test('errors thrown by getCss are handled', async () => {
        ...
        const response = await app.request('/v1/api/get-css?url=thisdoesnotexist.com');
        expect.soft(response.status).toBe(400);
      });
