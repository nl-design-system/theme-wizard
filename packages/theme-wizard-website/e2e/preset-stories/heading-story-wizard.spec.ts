...
test('wizard questions keep the intended order', async () => {
  await expect(wizard.legends).toHaveCount(7);
  ...
  await expect(wizard.legends.nth(6)).toHaveText('Kies de kleur van headings');
});
...
test('heading color preset has 4 options (Standaard + 3 accentkleuren)', async () => {
  const count = await wizard.waitForOptions('Kies de kleur van headings');
  expect(count).toBe(4);
});
...
test('changing heading color preset updates the theme token', async () => {
  const groupLabel = 'Kies de kleur van headings';
  const tokenPath = 'nl.heading.level-1.color';
